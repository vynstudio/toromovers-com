      ...
      <main id="main" className="w-full min-w-0 flex-1">
        ...
        <section className="full-bleed w-full" style={{ padding: "3rem 0" }}>
          <div className="site-container" style={{ maxWidth: "48rem" }}>
            ...
            <p style={{ marginTop: "1rem", fontWeight: 700 }}>You provide</p>
            <ul style={{ paddingLeft: "1.25rem" }}>
              {page.included.youProvide.map((b) => (
                <li key={b} className="text-muted" style={{ marginBottom: "0.4rem" }}>
                  {b}
                </li>
              ))}
            ...
            <p style={{ marginTop: "1rem", fontWeight: 700 }}>
              Share these when you call or request a quote:
            </p>
            <ul style={{ paddingLeft: "1.25rem" }}>
            ...
            <p className="text-muted" style={{ marginTop: "1rem" }}>
              {page.access.note} For apartment-focused full moves, see our{" "}
              <a href="/services">services hub</a> and{" "}
            ...
            <ul style={{ marginTop: "1rem", paddingLeft: "1.25rem" }}>
              {page.why.items.map((item) => (
                <li key={item.title} className="text-muted" style={{ marginBottom: "0.5rem" }}>
                  <strong>{item.title}</strong> — {item.body}
                </li>
              ))}
            </ul>
            <p className="text-muted" style={{ marginTop: "1rem" }}>
              See real job photos in the{" "}
              <a href="/orlando-movers-gallery">Orlando movers gallery</a>.
            </p>

            <section id="faq" style={{ marginTop: "2.5rem" }}>
              <h2 className="about-how-title">
                Labor-only movers — common questions
              </h2>
              <dl style={{ marginTop: "1rem" }}>
                {page.faqs.map((item) => (
                  <div key={item.q} style={{ marginBottom: "1.25rem" }}>
                    <dt style={{ fontWeight: 800 }}>{item.q}</dt>
                    <dd className="text-muted" style={{ marginTop: "0.35rem", marginLeft: 0 }}>
                      {item.a}
                    </dd>
                  </div>
                ))}
            ...
            >
              ...
              >
                ...
                <a
                  href="/services"
                  data-cta="labor-only-close-services"
