...
function FullAddress({ address, unit }: { address: string; unit?: string }) {
  if (!address.trim()) return null;
  if (!isFullStreetAddress(address)) {
    return <p className="mdc-hint">Tap a suggestion. We save the street, city, state, and ZIP.</p>;
  }
  return (
    <p className="mdc-captured">Saved: {addressWithUnit(address, unit || "")}</p>
  );
}
...
function StepDetails({
  ...
  return (
    <>
      ...
      <Field label="Pickup address" hint="Tap a suggestion.">
        <AddressAutocomplete
          streetOnly
      ...
      <FullAddress address={data.pickupAddress} unit={data.pickupUnit} />
      <Field label="Pickup unit, if any">
        <TextInput
      ...
      <Field label="Delivery address" hint="Tap a suggestion.">
        <AddressAutocomplete
          streetOnly
      ...
      <FullAddress address={data.deliveryAddress} unit={data.deliveryUnit} />
      <Field label="Delivery unit, if any">
        <TextInput
      ...
      <Field label="Will you be there at pickup?">
        <ChoiceGrid
          options={YES_NO}
      ...
      <Field label="Will you be there at delivery?">
        <ChoiceGrid
          options={YES_NO}
      ...
      {data.additionalStop === "Yes" ? (
        <>
          ...
          <FullAddress address={data.extraStopAddress} />
    ...
    <>
      ...
      <div className="mdc-review">
        ...
        <ReviewBlock title="Move details" onEdit={() => onEdit(1)}>
          ...
          <p>Pickup: {addressWithUnit(data.pickupAddress, data.pickupUnit) || "—"}</p>
          <p>Delivery: {addressWithUnit(data.deliveryAddress, data.deliveryUnit) || "—"}</p>
