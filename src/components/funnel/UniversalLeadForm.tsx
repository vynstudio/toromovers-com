...
const services: Array<{ value: ServiceType; title: string; description: string; icon: string }> = [
  ...
  { value: "labor_only", title: "Labor Only", description: "Loading, unloading, or moving help", icon: "💪" },
  { value: "same_building_move", title: "Same-Building Move", description: "Moving within one building or complex", icon: "🏢" },
  { value: "special_item_move", title: "Special Item Move", description: "Piano, safe, appliance, or heavy furniture", icon: "🎹" },
  { value: "pod_storage_container", title: "POD / Storage Container", description: "Load or unload a POD or container", icon: "📦" },
  { value: "rental_truck_labor", title: "U-Haul / Rental Truck", description: "You provide the truck; we provide the movers", icon: "🚐" },
  { value: "single_item_move", title: "Single-Item Move", description: "One item, furniture pickup, or a small move", icon: "🛋️" },
];
...
export default function UniversalLeadForm({ source = "ads_landing_page" }: { source?: string }) {
  ...
  useEffect(() => { captureAttribution(); trackFunnelEvent("form_start", { form_location: source }); }, [source]);
  ...
  async function submit(event: FormEvent) {
    ...
    if (!name.trim() || !email.trim() || !phoneE164 || !consent) {
    ...
    try {
      ...
      setError("We could not submit your request. Please call (321) 758-0094 and our team will help right away.");
      setSubmitting(false);
    }
  }
  ...
  return <section id="quote-form" className="rounded-3xl bg-white p-5 shadow-2xl ring-1 ring-black/5 sm:p-8">
    <div className="mb-6 flex items-center justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-widest text-zinc-500">Free moving quote</p><h2 className="mt-1 text-2xl font-black tracking-tight">{step === 1 ? "What kind of moving help do you need?" : step === 2 ? "Tell us about the move" : step === 3 ? "Where and when are you moving?" : "How can we reach you?"}</h2></div><span className="text-sm font-bold text-zinc-500">Step {step} of 4</span></div>
    ...
    {step === 1 && <div className="grid gap-3">{services.map((item) => <button type="button" key={item.value} onClick={() => chooseService(item.value)} className="group flex items-center gap-4 rounded-2xl border border-zinc-200 p-4 text-left transition hover:border-black hover:bg-zinc-50"><span className="grid h-12 w-12 place-items-center rounded-xl bg-zinc-100 text-2xl group-hover:bg-black group-hover:text-white">{item.icon}</span><span><span className="block font-extrabold">{item.title}</span><span className="mt-0.5 block text-sm text-zinc-600">{item.description}</span></span></button>)}</div>}
